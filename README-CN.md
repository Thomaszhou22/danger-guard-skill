...
安装后，Danger Guard 会自动将拦截规则写入 Agent 的必读文件：

| 目标文件 | 写入内容 | 原因 |
|---------|---------|------|
| `MEMORY.md` | 顶部写入危险关键词速查 | 每次 session 必读，最高优先级 |
| `AGENTS.md` | 完整关键词触发列表 | 行为规范层 |
| `danger-guard-config.json` | `hardening_complete: true` | 标记加固完成 |

这确保即使 Agent 没有主动加载 Skill，拦截也能生效。无需手动操作，安装后首次运行时自动完成。
...
│  (MEMORY.md + AGENTS.md)                      │
│                                               │
│  危险命令关键词直接写入 Agent 每次必读的文件。   │
│  确保 Agent 没有主动加载 Skill 时也能拦截。      │
├───────────────────────────────────────────────┤
│  第 1 层：AI 智能体保护                         │
│  (OpenClaw Skill / Claude Code 等)             │
│                                               │
│  拦截通过 AI 工具和聊天平台发送的危险命令         │
├───────────────────────────────────────────────┤
│  第 2 层：系统级保护                            │
│  (Shell Wrapper — 可选)                        │
│                                               │
...
Skill 是被动文档，只有 Agent 主动加载时才生效。如果 Agent 的注意力被其他事物占据（上下文过长、命令不完整、不相关话题），可能不会加载 Skill，拦截就会失败。

**安装加固**（v1.1.0 新增）解决了这个问题：把危险命令关键词直接写入 `MEMORY.md` 和 `AGENTS.md`，这些文件 Agent 每次 session 都会读取。这保证了无论 Skill 是否被加载，拦截都能生效。

| 层级 | 保护内容 | 保障 |
|------|---------|------|
| **记忆加固** | 强制 Agent 识别危险关键词 | 每次 session 必读 |
